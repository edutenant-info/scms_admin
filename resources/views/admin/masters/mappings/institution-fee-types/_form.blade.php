@php
    /** @var \App\Models\InstitutionFeeType $item */
@endphp
...
<div class="cd"><div class="cb">
    <div class="fgrid">
        ...
        <div class="fg">
            <label>Fee Type <span style="color:var(--ros);">*</span></label>
            <select name="fee_type_id" class="fs">
                <option value="">Select fee type…</option>
                @foreach ($feeTypes as $id => $name)
                    <option value="{{ $id }}" @selected((int) old('fee_type_id', $item->fee_type_id) === (int) $id)>{{ $name }}</option>
                @endforeach
            </select>
            @error('fee_type_id')<div style="color:var(--ros);font-size:11px;margin-top:4px;">{{ $message }}</div>@enderror
        </div>
    </div>
...
<div style="display:flex;justify-content:flex-end;gap:9px;margin-top:20px;">
    <a href="{{ route('admin.institution-fee-types.index') }}" class="btn bo">Cancel</a>
