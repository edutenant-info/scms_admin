@php
    /** @var \App\Models\InstitutionType $type */
    $val = fn ($field, $default = null) => old($field, $type->{$field} ?? $default);
@endphp
...
<div class="cd"><div class="cb">
    <div class="fgrid">
        <div class="fg">
            ...
            <input type="text" name="name" class="fi noi" value="{{ $val('name') }}" minlength="3" maxlength="100" placeholder="e.g. Engineering College">
    ...
    <div style="margin-top:18px;padding-top:18px;border-top:1px solid var(--bdr);">
        <label style="display:flex;align-items:center;gap:10px;font-size:13px;color:var(--t2);cursor:pointer;">
            <span class="sw"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $type->is_active ?? true))><span class="sl"></span></span>
            Active
        </label>
    </div>
</div></div>
...
<div style="display:flex;justify-content:flex-end;gap:9px;margin-top:20px;">
    <a href="{{ route('admin.institution-types.index') }}" class="btn bo">Cancel</a>
    <button type="submit" class="btn ba"><i class="fa-solid fa-save"></i> {{ $type->exists ? 'Update' : 'Create' }} Type</button>
</div>
